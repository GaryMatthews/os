...
// Cursor stuff
// -----------------------------------------------------------------------------
static uint8_t cursorEnabled = 0;
...
void vga_update_cursor(uint16_t x, uint16_t y) {
	...
	// Set cursor position.
	outportb(VGA_CRTC_ADDRESS, VGA_REG_CURSOR_LOC_LOW);
	outportb(VGA_CRTC_DATA, (uint8_t)(pos & 0xFF));
	outportb(VGA_CRTC_ADDRESS, VGA_REG_CURSOR_LOC_HIGH);
	outportb(VGA_CRTC_DATA, (uint8_t)((pos >> 8) & 0xFF));
}
...
void vga_enable_cursor(void) {
	...
	outportb(VGA_CRTC_ADDRESS, VGA_REG_CURSOR_START);
	outportb(VGA_CRTC_DATA, VGA_CURSOR_SCANLINE_START);
 
	outportb(VGA_CRTC_ADDRESS, VGA_REG_CURSOR_END);
	outportb(VGA_CRTC_DATA, VGA_CURSOR_SCANLINE_END);
	cursorEnabled = 1;
...
void vga_disable_cursor(void) {
	// Disable cursor.
	outportb(VGA_CRTC_ADDRESS, VGA_REG_CURSOR_START);
	outportb(VGA_CRTC_DATA, (inportb(VGA_CRTC_DATA) & VGA_CURSOR_DISABLE));
	cursorEnabled = 0;
}
...
void vga_writes(const char* data) {
	for (uint32_t i = 0; i < (uint32_t)strlen(data); i++)
		vga_putchar(data[i]);
}
...
void vga_init(void) {
	...
	terminalBuffer = (uint16_t*)(VGA_MEMORY_ADDRESS);
